...
| Calendar colors (1-bit) | `cal_map_bw` | the map a black-and-white device uses: `black`, `white`, `gray-10`..`gray-75` (the number is the lightness; `gray-75` is nearly white). Empty maps the colour map automatically: yellow tints to white, red to gray-35, orange to gray-60, pink to gray-70 |
...
- **A few blends are worth using as fills.** `orange` is a red-and-yellow tile, `pink` a red-and-white one, and a `-45`..`-75` step of a hue (say `yellow-60`) is that hue's dots on a white ground. The `-10`..`-40` steps put black dots on the solid hue. Judge them only at 1:1 pixels (the device, or the settings-page render PNG opened at full size): a `-50`/`-60` tint is a one-pixel checkerboard, and any downscaled view of it aliases into solid in one column and blank in the next, which is a viewer artefact and not the render. The busiest calendar takes the palest tint and the rare calendars the accents.
- **A 1-bit panel turns every hue into a grey dither, and a light tint into a dark one.** The framework's 1-bit theme maps `yellow-60` to a 70% black dither, `red` to 40%, `orange` to 55%; black text on those drowns. That is why `cal_map_bw` exists. The render cannot tell which device it is for (`trmnl.device.friendly_id` names one device whichever screen is drawn), but the framework marks a 1-bit or 2-bit screen with a class, so the grey map is emitted as CSS scoped to `.screen--1bit` and `.screen--2bit` that swaps each calendar's fill and text variables. One plugin serves both panels.
