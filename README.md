...
| 1-bit devices | `bw_devices` | friendly ids of black-and-white devices, one per line; on those the next map applies |
| Calendar colors (1-bit) | `cal_map_bw` | the map for 1-bit devices: `black`, `white`, `gray-10`..`gray-75` (the number is the darkness). Empty maps the colour map automatically: yellow tints to white, red to gray-65, orange to gray-30, pink to gray-25 |
...
- **A 1-bit panel turns every hue into a grey dither, and a light tint into a dark one.** The framework's 1-bit theme maps `yellow-60` to a 70% black dither, `red` to 40%, `orange` to 55%; black text on those drowns. That is why `bw_devices` and `cal_map_bw` exist: one plugin serves both panels, and the render picks the map by `trmnl.device.friendly_id`.
